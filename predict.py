...
file=pd.read_csv('diabetes_data_upload.csv')
...
Alopecia='No'
Obesity='No'
new_entry=[30,'Male','No','No','No','No','No','Yes','No','No','Yes','No','No','No','No','No']
print(new_entry)
...
for m_impurity in impurity_measure:
    ...
    for dataset in datasets:
        ...
        if best_accuracy < accuracy:
            best_tree = tree
    print('Scores from', m_impurity, ' are : ', accuracies)
    print('Mean Accuracy for', m_impurity,' is :',(sum(accuracies)/float(len(accuracies))))
    prediction=predict_value(best_tree, new_entry)
    print("If the Person is prone to diabetes he is positive else Negative and the results is ",prediction)
